...
(() => {
  ...
  function initLoader() {
    ...
    const hideLoader = () => {
      if (dom.loader.getAttribute("aria-hidden") === "true") return;
      dom.loader.style.opacity = "0";
      dom.loader.style.pointerEvents = "none";
      setTimeout(() => {
        dom.loader.style.display = "none";
        dom.loader.setAttribute("aria-hidden", "true");
        // Force AOS refresh multiple times as elements settle
        if (window.AOS) {
          AOS.refresh();
    ...
    // Safety fallback: hide loader after 3s anyway
  ...
  function initAOS() {
    if (window.AOS && typeof AOS.init === "function") {
      ...
      // Safety: refresh AOS on first scroll
